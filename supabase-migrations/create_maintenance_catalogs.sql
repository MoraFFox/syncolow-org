-- =============================================================================
-- Maintenance Catalog Tables Migration
-- =============================================================================
-- Creates tables for persisting maintenance catalogs:
-- - maintenance_service_catalog: Services that can be performed
-- - maintenance_parts_catalog: Spare parts inventory
-- - maintenance_problems_catalog: Common problem types
-- =============================================================================
...
-- =============================================================================
CREATE TABLE IF NOT EXISTS mock_data.maintenance_service_catalog_data (
...
CREATE OR REPLACE VIEW mock_data."maintenanceServiceCatalog" AS
SELECT
    id,
    category,
    name,
...
FROM mock_data.maintenance_service_catalog_data;

-- =============================================================================
-- Spare Parts Catalog Table
-- =============================================================================
CREATE TABLE IF NOT EXISTS mock_data.maintenance_parts_catalog_data (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    category TEXT NOT NULL,
    name TEXT NOT NULL,
...
CREATE OR REPLACE VIEW mock_data."maintenancePartsCatalog" AS
SELECT
    id,
    category,
    name,
    ...
    created_at AS "createdAt",
    updated_at AS "updatedAt"
FROM mock_data.maintenance_parts_catalog_data;

-- =============================================================================
-- Problems Catalog Table
-- =============================================================================
CREATE TABLE IF NOT EXISTS mock_data.maintenance_problems_catalog_data (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    category TEXT NOT NULL,
...
-- Create view with camelCase columns
CREATE OR REPLACE VIEW mock_data."maintenanceProblemsCatalog" AS
SELECT
    id,
    category,
    problem,
    ...
    created_at AS "createdAt",
    updated_at AS "updatedAt"
FROM mock_data.maintenance_problems_catalog_data;

-- =============================================================================
-- Indexes for better query performance
-- =============================================================================
CREATE INDEX IF NOT EXISTS idx_service_catalog_category 
    ON mock_data.maintenance_service_catalog_data(category);
CREATE INDEX IF NOT EXISTS idx_service_catalog_active 
    ON mock_data.maintenance_service_catalog_data(is_active) WHERE is_active = TRUE;

CREATE INDEX IF NOT EXISTS idx_parts_catalog_category 
    ON mock_data.maintenance_parts_catalog_data(category);
CREATE INDEX IF NOT EXISTS idx_parts_catalog_active 
    ON mock_data.maintenance_parts_catalog_data(is_active) WHERE is_active = TRUE;

CREATE INDEX IF NOT EXISTS idx_problems_catalog_category 
    ON mock_data.maintenance_problems_catalog_data(category);
CREATE INDEX IF NOT EXISTS idx_problems_catalog_active 
    ON mock_data.maintenance_problems_catalog_data(is_active) WHERE is_active = TRUE;

-- =============================================================================
-- Seed initial data from existing hardcoded catalogs
-- =============================================================================

-- Services Catalog Seed Data
INSERT INTO mock_data.maintenance_service_catalog_data (category, name, default_cost) VALUES
...
-- Parts Catalog Seed Data
INSERT INTO mock_data.maintenance_parts_catalog_data (category, name, default_price) VALUES
...
-- Problems Catalog Seed Data
INSERT INTO mock_data.maintenance_problems_catalog_data (category, problem, severity) VALUES
...
-- =============================================================================
ALTER TABLE mock_data.maintenance_service_catalog_data ENABLE ROW LEVEL SECURITY;
ALTER TABLE mock_data.maintenance_parts_catalog_data ENABLE ROW LEVEL SECURITY;
ALTER TABLE mock_data.maintenance_problems_catalog_data ENABLE ROW LEVEL SECURITY;

-- Allow all operations for authenticated users (adjust based on your auth setup)
CREATE POLICY "Allow all for authenticated users" ON mock_data.maintenance_service_catalog_data
    FOR ALL USING (true);
    
CREATE POLICY "Allow all for authenticated users" ON mock_data.maintenance_parts_catalog_data
    FOR ALL USING (true);
    
CREATE POLICY "Allow all for authenticated users" ON mock_data.maintenance_problems_catalog_data
    FOR ALL USING (true);
