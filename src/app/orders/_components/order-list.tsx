...
import { MoreHorizontal, XCircle, ArrowUp, ArrowDown } from 'lucide-react';
...
export function OrderList({
    ...
    if (!sortConfig) {
        console.error('OrderList: sortConfig is missing!');
                    ...
                    <CardContent className="p-4 flex flex-col gap-2">
                        ...
                        <div className="flex justify-between items-end">
                            <div>
                                ...
                                <p className="text-lg font-bold">{formatCurrency(order.grandTotal || order.total || 0)}</p>
                            </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>
                                    <Button variant="ghost" onClick={() => onSort('id')} className="flex items-center gap-1 p-0 hover:bg-transparent -ml-2">
                                        Order ID
                                        {sortConfig.key === 'id' && (sortConfig.direction === 'asc' ? <ArrowUp className="w-4 h-4 ml-1" /> : <ArrowDown className="w-4 h-4 ml-1" />)}
                                    </Button>
                                </TableHead>
                                <TableHead>
                                    <Button variant="ghost" onClick={() => onSort('client')} className="flex items-center gap-1 p-0 hover:bg-transparent -ml-2">
                                        Client
                                        {sortConfig.key === 'client' && (sortConfig.direction === 'asc' ? <ArrowUp className="w-4 h-4 ml-1" /> : <ArrowDown className="w-4 h-4 ml-1" />)}
                                    </Button>
                                </TableHead>
                                <TableHead>
                                    <Button variant="ghost" onClick={() => onSort('date')} className="flex items-center gap-1 p-0 hover:bg-transparent -ml-2">
                                        Date
                                        {sortConfig.key === 'date' && (sortConfig.direction === 'asc' ? <ArrowUp className="w-4 h-4 ml-1" /> : <ArrowDown className="w-4 h-4 ml-1" />)}
                                    </Button>
                                </TableHead>
                                <TableHead>
                                    <Button variant="ghost" onClick={() => onSort('deliveryDate')} className="flex items-center gap-1 p-0 hover:bg-transparent -ml-2">
                                        Delivery Date
                                        {sortConfig.key === 'deliveryDate' && (sortConfig.direction === 'asc' ? <ArrowUp className="w-4 h-4 ml-1" /> : <ArrowDown className="w-4 h-4 ml-1" />)}
                                    </Button>
                                </TableHead>
                                <TableHead>
                                    <Button variant="ghost" onClick={() => onSort('status')} className="flex items-center gap-1 p-0 hover:bg-transparent -ml-2">
                                        Status
                                        {sortConfig.key === 'status' && (sortConfig.direction === 'asc' ? <ArrowUp className="w-4 h-4 ml-1" /> : <ArrowDown className="w-4 h-4 ml-1" />)}
                                    </Button>
                                </TableHead>
                                <TableHead className="text-right">
                                    <Button variant="ghost" onClick={() => onSort('total')} className="flex items-center gap-1 p-0 hover:bg-transparent w-full justify-end">
                                        Total
                                        {sortConfig.key === 'total' && (sortConfig.direction === 'asc' ? <ArrowUp className="w-4 h-4 ml-1" /> : <ArrowDown className="w-4 h-4 ml-1" />)}
                                    </Button>
                                </TableHead>
                        ...
                        <TableBody>
                            {orders.map(order => (
                                <TableRow key={order.id} data-state={selectedRowKeys.has(order.id) && "selected"}>
                                    ...
                                    <TableCell className="text-right cursor-pointer" onClick={() => router.push(`/orders/${order.id}`)}>{formatCurrency(order.grandTotal || order.total || 0)}</TableCell>
