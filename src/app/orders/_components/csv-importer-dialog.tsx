...
export function CsvImporterDialog({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
   ...
   const runImport = useCallback(async (dataToImport: CsvRow[], isRerun: boolean = false) => {
     const CHUNK_SIZE = 500; // Process 500 rows at a time to stay under 1MB limit
  ...
  const handleFixAll = async () => {
    ...
    try {
      const missingEntityErrors = errors.filter(e => e.errorType === 'missing-entity');
      ...
      for (const error of missingEntityErrors) {
        if (error.resolution?.entity === 'company' && error.resolution.suggestedData) {
          ...
          const existingCompany = await getDocs(
            query(collection(db, 'companies'), where('name', '==', error.resolution.suggestedData.name))
          );
          
          if (existingCompany.empty) {
            const companyData = {
              ...error.resolution.suggestedData,
              autoGenerated: true,
              needsMoreInfo: true,
            ...
            createdCompanies.push(error.resolution.suggestedData.name);
          }
        } else if (error.resolution?.entity === 'product' && error.resolution.suggestedData) {
          // Check if product already exists
          const existingProduct = await getDocs(
            query(collection(db, 'products'), where('name', '==', error.resolution.suggestedData.name))
          );
          ...
          if (existingProduct.empty) {
            ...
            createdProducts.push(error.resolution.suggestedData.name);
          }
        }
      }
      
      setAutoGeneratedEntities({ companies: createdCompanies, products: createdProducts });
      toast({ 
        title: 'Entities Created', 
        description: `Created ${createdCompanies.length} companies and ${createdProducts.length} products. Retrying import...` 
      });
