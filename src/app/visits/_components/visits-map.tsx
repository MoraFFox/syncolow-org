    ...
    const geocodeVisits = async () => {
      ...
      const geocoded = await Promise.all(
        visits.map(async (visit) => {
          const coords = visit.address
            ? await geocodeService.geocode(visit.address)
            : null;
          return { ...visit, coords };
        })
      );
      ...
      if (geocoded.length > 0) {
        const firstValidVisit = geocoded.find(
          (v): v is VisitWithCoords & { coords: LatLngTuple } =>
            v.coords !== null
        );
        if (firstValidVisit) {
          if (
            !selectedVisitId ||
            !geocoded.find((v) => v.id === selectedVisitId)
          ) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Geographical overview of upcoming visits.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <GoogleMapWrapper
