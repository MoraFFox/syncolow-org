...
import type { AlertItem, AlertPriority, AlertType, AlertData, OverdueOrder } from "./types";
...
export function formatAlertMessage(alert: { type: AlertType; data: AlertData }) {
  ...
  if (alert.type === "Overdue Payment") return `Payment for ${(alert.data as OverdueOrder).companyName} is overdue.`;
  return "";
}
