...
const ActivityDetails = ({ activity }: { activity: Activity }) => {
  switch (activity.type) {
    case "New Order":
      const order = activity.data as Pick<
        Order,
        "id" | "companyId" | "companyName" | "orderDate"
      >;
      return (
        <p className='text-sm'>
          New order from{" "}
          <DrillTarget
            kind='company'
            payload={{ id: order.companyId, name: order.companyName }}
            asChild
          >
    ...
    case "New Feedback":
      const feedback = activity.data as Pick<
        Feedback,
        "id" | "clientId" | "message" | "feedbackDate"
      >;
      return (
        <p className='text-sm'>
          New feedback from{" "}
          ...
          : "{feedback.message?.slice(0, 30)}..."
        </p>
      );
    case "New Client":
      const client = activity.data as Pick<
        Company,
        "id" | "name" | "createdAt"
      >;
      return (
...
const ActivityLink = ({ activity }: { activity: Activity }) => {
  switch (activity.type) {
    case "New Order":
      return `/orders/${activity.data.id}`;
    case "New Feedback":
      return `/feedback`;
    case "New Client":
      return `/clients/${activity.data.id}`;
    default:
      return "#";
  }
};
...
export function ActivityFeed() {
  ...
  return (
    <SectionCard
      ...
      description='A feed of the latest events in your system.'
      loading={isLoading}
    >
      {isLoading ? (
        ...
        <div className='space-y-3'>
          {recentActivities && recentActivities.length > 0 ? (
            ...
            <EmptyState
              ...
              description='New orders, clients, and feedback will appear here.'
            />
          )}
        </div>
      )}
