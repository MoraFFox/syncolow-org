...
function CustomDatesField({ control, register }: any) {
  ...
  return (
    <div className="grid gap-2">
      ...
      <p className="text-sm text-muted-foreground mb-2">Add dates that repeat every year (MM-DD format)</p>
      ...
      <p className="text-xs text-muted-foreground">Example: 01-01 (Jan 1), 08-01 (Aug 1)</p>
    </div>
  );
}

export function Step1_CompanyDetails({ control, register, errors, openMapPicker, setValue, watch, isWizard = true, isEditMode = false }: any) {
    ...
    <div className="space-y-6">
        <div className="space-y-4">
            <div className="grid gap-2">
                ...
                <Input id="name" {...register("name")} placeholder="e.g. The Coffee House"/>
            ...
            {isEditMode && (!watch('branches') || watch('branches')?.length === 0) && (
                <div className="grid gap-2">
                    ...
                    <div className="flex items-center space-x-2 mb-2">
                        ...
                        />
                        <Label htmlFor="companyMachineOwned">Client Owns the Machine</Label>
                    </div>
                    {!watch('machineOwned') && (
                        <>
                            <Controller
                            ...
                            {watch('machineLeased') && (
                                <div className="grid gap-2 mt-2">
                                    <Label htmlFor="leaseMonthlyCost">Monthly Lease Cost (EGP)</Label>
                                    <Input
                                        id="leaseMonthlyCost"
                                        type="number"
                                        min="0"
                                        {...register('leaseMonthlyCost', { valueAsNumber: true })}
            ...
            <div className="grid gap-2">
                ...
                <Button type="button" variant="outline" onClick={() => openMapPicker('company')}>
                    <MapPin className="mr-2 h-4 w-4" />
                    {watch('location') ? 'Change Location' : 'Set Location on Map'}
                </Button>
                {watch('location') && <p className="text-sm text-muted-foreground truncate">{watch('location')}</p>}
          ...
          <>
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Set payment terms for this company</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="grid gap-2">
                            ...
                            render={({ field }) => (
                                <Select onValueChange={field.onChange} value={field.value}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="days_after_order">After order</SelectItem>
                                        <SelectItem value="monthly_date">Monthly on specific date</SelectItem>
                                        <SelectItem value="bulk_schedule">Bulk payment schedule</SelectItem>
                                    </SelectContent>
                                </Select>
                            )}
                        />
                ...
                {watch('paymentDueType') === 'days_after_order' && (
                    <div className="grid gap-2">
                        ...
                        <Input 
                            ...
                            placeholder="e.g. 30"
                        />
                        {errors.paymentDueDays && <p className="text-sm text-destructive">{errors.paymentDueDays.message}</p>}
                    </div>
                )}
                {watch('paymentDueType') === 'monthly_date' && (
                    <div className="grid gap-2">
                        ...
                        <Input 
                            ...
                            placeholder="e.g. 15"
                        />
                        {errors.paymentDueDate && <p className="text-sm text-destructive">{errors.paymentDueDate.message}</p>}
                    </div>
                )}
                {watch('paymentDueType') === 'bulk_schedule' && (
                    <div className="space-y-4">
                        ...
                        {watch('bulkPaymentSchedule.frequency') !== 'custom' && watch('bulkPaymentSchedule.frequency') && (
                            <div className="grid gap-2">
                                <Label htmlFor="bulkPaymentDayOfMonth">Day of Month (1-31)</Label>
                                <Input 
                                    id="bulkPaymentDayOfMonth" 
                                    type="number" 
                                    min="1" 
                                    max="31"
                                    {...register("bulkPaymentSchedule.dayOfMonth", { valueAsNumber: true })} 
                                    placeholder="e.g. 15"
                                />
                            </div>
                        )}
            ...
            <Separator />
            <ContactsSubForm control={control} register={register} errors={errors} fieldNamePrefix="contacts" title="Company Contacts" description="Main contacts for the parent company." setValue={setValue}/>
          </>
        )}
    </div>
  );
