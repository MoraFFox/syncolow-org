      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            style={
              {
                "--sidebar-width": "var(--sidebar-width-dynamic, 16rem)",
...
>(
  (
    ...
    return (
      <div
        ref={ref}
        className="group peer hidden md:block border-r border-border/60 text-card-foreground"
        data-state={state}
...
>(({ className, ...props }, ref) => {
    ...
    React.useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            document.documentElement.style.setProperty('--sidebar-width-dynamic', `${newWidth}px`);
        };

        const handleMouseUp = () => {
            if (!isResizing) return;
            setIsResizing(false);
            const finalWidth = document.documentElement.style.getPropertyValue('--sidebar-width-dynamic');
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "peer-data-[variant=inset]:min-h-[calc(100svh-theme(spacing.4))] max-w-80vw",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-8 w-full bg-accent shadow-none focus-visible:ring-2 focus-visible:ring-ring",
        className
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-ring transition-[width,height,padding] hover:bg-accent group-hover/menu-item:bg-accent group-hover/menu-item:text-accent-foreground focus-visible:ring-2 active:bg-accent active:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-accent data-[active=true]:text-accent-foreground data-[active=true]:font-medium data-[state=open]:hover:bg-accent data-[state=open]:hover:text-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "data-[active=true]:bg-accent data-[active=true]:text-accent-foreground",
        outline:
