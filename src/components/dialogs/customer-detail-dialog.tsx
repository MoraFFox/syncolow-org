...
export function CustomerDetailDialog({ isOpen, onOpenChange }: CustomerDetailDialogProps) {
  ...
  React.useEffect(() => {
    ...
    const fetchCustomerData = async () => {
      ...
      try {
        ...
        if (companyResult.error || !companyResult.data) {
          ...
          const totalSpent = ordersResult.data?.reduce((sum, order) => sum + (order.grandTotal || 0), 0) || 0;
    ...
    fetchCustomerData();
  }, [isOpen, payload?.id, payload?.name]);
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {customerData?.name || payload?.name || 'Customer Details'}
          </DialogTitle>
          <DialogDescription>
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{formatCurrency(customerData.averageOrderValue)}</div>
                  <p className="text-xs text-muted-foreground">
                    Per transaction
                  </p>
                </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <Badge variant={getStatusColor(customerData.status)}>{customerData.status}</Badge>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Member Since</span>
                  <span className="text-sm font-medium">{customerData.joinedDate}</span>
                </div>
                <div className="flex justify-between">
