...
interface DrillDownState {
  ...
  // Bookmarks State
  bookmarks: Array<{ id: string; label: string; kind: DrillKind; payload: DrillPayload; createdAt: number }>;
  
  // Pinned Previews State
  pinnedPreviews: Array<{ id: string; kind: DrillKind; payload: DrillPayload; position: { x: number; y: number } }>;
...
export const useDrillDownStore = create<DrillDownState>((set, get) => ({
  ...
  addToCompare: (kind, payload) => set((state) => {
  ...
  addBookmark: (label, kind, payload) => set((state) => {
    const bookmark = {
  ...
  pinPreview: (kind, payload) => set((state) => {
